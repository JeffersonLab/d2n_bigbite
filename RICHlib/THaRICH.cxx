  ...
  fTrackX(kBig), fTrackY(kBig)
{
  ...
  fHits             = new TClonesArray("THaRICH_Hit",1000);
  fClusters         = new TClonesArray("THaRICH_Cluster",100);
  fResolvedHits     = new TClonesArray("THaRICH_Hit",1000);
  fResolvedClusters = new TClonesArray("THaRICH_Cluster",100);
...
//_____________________________________________________________________________
void THaRICH::Clear( Option_t* opt )
{
...
//_____________________________________________________________________________
Int_t THaRICH::ReadDatabase( const TDatime& date )
{
  // 
...
//_____________________________________________________________________________
Int_t THaRICH::DefineVariables( EMode mode )
{
  ...
  DefineVarsFromList( var2, mode, "fHits.THaRICH_Hit." );
  ...
  DefineVarsFromList( var3, mode, "fResolvedHits.THaRICH_Hit." );
  ...
  DefineVarsFromList( var4, mode, "fClusters.THaRICH_Cluster." );
  ...
  DefineVarsFromList( var5, mode, "fResolvedClusters.THaRICH_Cluster." );
...
//_____________________________________________________________________________
//
Double_t THaRICH::Cherenkov_Angle(double mass, double momentum)  const
{
//
...
Int_t THaRICH::FineProcess( TClonesArray& tracks )
{
  ...
  for( int k = 0; k < (nClusters+nResolvedClusters); k++ ) {
    ...
    Int_t imip = -1;
    THaRICH_Cluster* theMIP_mindist = NULL;
    for( int i = 0; i<nTracks; i++ ) {
      if( !fMIPs[i] ) continue;
    ...
    for( int i = 0; i<nTracks; i++ ) {
      ...
      THaRICH_Cluster* theMIP = fMIPs[i];
...
//
//**************************************************************************
void THaRICH::Padn2xy( Int_t i_pad, Int_t j_pad, Double_t charg ) 
{
  ...
  THaRICH_Hit* h = new( (*fHits)[i] ) 
    THaRICH_Hit( i, icharg, i_pad, j_pad, 
		 i_X + PAD_SIZE_X/2,
...
Int_t THaRICH::FindClusters()
{
  ...
  for( int k=0; k<nHits; k++ ) {
    ...
    if( theHit->GetFlag() == 0 ) {
      ...
      theCluster = new( (*fClusters)[nClust++] ) THaRICH_Cluster();
      theHit->SetFlag(1);
...
Int_t THaRICH::FindMIP( const TClonesArray& tracks )
{
  ...
  fMIPs  = new THaRICH_Cluster* [ ntracks ];
  ...
  for( Int_t k=0; k<ntracks; k++ ) {
    ...
    THaRICH_Cluster* theCluster;
    THaRICH_Cluster* minDistClust   = NULL;
    THaRICH_Cluster* maxChargeClust = NULL;
    THaRICH_Cluster* theMIP         = NULL;
...
Int_t THaRICH::ResolveClusters()
{
  ...
  for( int k = 0; k < nClust; k++ ) {
    THaRICH_Cluster* theCluster = GetCluster(k);
    ...
    if( (Number > 1) && !(theCluster->IsMIP()) ) {
      ...
      while( const THaRICH_Hit* pHit = static_cast<THaRICH_Hit*>(next())) {
	if( pHit->GetVeto() != 0 ) continue;
	// found a local maximum
	THaRICH_Cluster* the_Resolved_Cluster = 
	  new( (*fResolvedClusters)[nResolvedClusters++] ) THaRICH_Cluster();
...
// the C-photon.
//**************************************************************************
Double_t THaRICH::Get_phi_photon( Double_t x_photon, Double_t y_photon,
...
//and the origin.
//***************************************************************************
Double_t THaRICH::Get_a( Double_t theta_mip, Int_t Calculation_kind) const
{
...
//                      Get_phi_photon
//***************************************************************************
Double_t THaRICH::Get_b( Double_t x_photon, Double_t y_photon,
			Double_t x_mip, Double_t y_mip,
			Double_t theta_mip, Double_t phi_mip, 
			Int_t Calculation_kind) const
{
...
//***************************************************************************
Double_t THaRICH::Get_theta_photon(Double_t x_photon, Double_t y_photon,
				  Double_t x_mip, Double_t y_mip,
				  Double_t theta_mip, Double_t phi_mip, 
				  Int_t Calculation_kind)  const
{
...
Double_t THaRICH::RecoAng( Double_t x_photon, Double_t y_photon,
...
Int_t THaRICH::ClearNoise(Int_t igold, Int_t ResolvedFlag)
{
  Int_t success = 0;
  Int_t Index;
  ...
  for( int k = istart; k < istop; k++ ) 
	...
	Proton_success = 1;
      ...
      fMIP.Insert_N_chi2_corrected_Photon(2, Proton_chi2_degree_of_freedom-1,
					  ResolvedFlag);
      Index = Pion_Index;
      THaRICH_Cluster* theCluster = ( Index < nClusters ) ? GetCluster(Index) 
...
#if 0
Int_t THaRICH::FineProcess_old( TClonesArray& tracks )
{
  // The main RICH processing method. Here we
  //  
  // - Identify clusters
  ...
  THaRICH_Cluster* theCluster = fClusters;
...
void THaRICH::ReadBadPads( Char_t *infilename )
{
...
//_____________________________________________________________________________
void THaRICH::PrintBenchmarks() const
{
