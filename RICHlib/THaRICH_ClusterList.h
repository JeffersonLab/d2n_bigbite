#ifndef ROOT_THaRICH_ClusterList
#define ROOT_THaRICH_ClusterList
...
class THaRICH_Hit : public TObject {

public:
  THaRICH_Hit() 
    : fFlag(0), fVeto(0) {}
  THaRICH_Hit( Int_t number, Int_t ADC, Int_t i, Int_t j, 
	       Float_t x, Float_t y ) :
...
private:
  ...
  ClassDef(THaRICH_Hit,0)   //A hit in the RICH
};
...
class THaRICH_Cluster : public TObject {

public:
  THaRICH_Cluster();
  THaRICH_Cluster( const THaRICH_Cluster& rhs );
  THaRICH_Cluster& operator=( const THaRICH_Cluster& rhs );

  ~THaRICH_Cluster() { delete fHitList; }


  void       Clear( Option_t* opt="" );
  Float_t    Dist( const THaRICH_Cluster* c ) const;
  void       Insert( THaRICH_Hit* theHit );
  void       Insert( THaRICH_Hit* theHit, Float_t factor);
  ...
  Int_t FindResolvedClusterElements (const THaRICH_Hit* localMaximum,
				     THaRICH_Cluster* resolvedCluster,
  ...
  void       SetMIP( THaRICH_Cluster* mip )  { fMIP = mip; }
  ...
  Int_t      Test( const THaRICH_Hit* theHit,  Float_t par1, 
...
private:
  ...
  THaRICH_Cluster* fMIP; //Pointer to MIP cluster belonging to this cluster
  ...
  ClassDef(THaRICH_Cluster,0)  //A cluster of hits in the RICH
};
...
inline
void THaRICH_Cluster::MakeMIP( Bool_t flag )
{
  fMIPflag = flag;
  if( flag )
    fMIP = this;
