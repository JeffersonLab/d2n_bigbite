...
TString guiDirectory = "onlineGUI_v1.2/";
...
//   Int_t dim = Int_t(round(sqrt(double(draw_count))));
  ...
  for(UInt_t i=0; i<draw_count; i++) {    
    ...
    thiscommand = fileObject2command(thiscommand);
    fCanvas->cd(i+1);
...
//     CheckRootFile();
  ...
  while((key=(TKey*)next())!=0) {
    ...
    if(objtype.Contains("TDirectory")) {
      TDirectory *thisdir = (TDirectory*)fRootFile->Get(objname);
      ...
      while((innerkey=(TKey*)nextInDir())!=0) {
	      ...
	      vfileObjects.push_back(thisObject);
	    }
    } else {
      fileObject thisObject;
...
void OnlineGUI::GetRootTree() {
  ...
  list <TString> found;
  for(UInt_t i=0; i<vfileObjects.size(); i++) {
#ifdef DEBUGGETROOTTREE
    cout << "Object = " << vfileObjects[i].type <<
      "     Name = " << vfileObjects[i].name << endl;
#endif
    if(vfileObjects[i].type.Contains("TTree"))
       found.push_back(vfileObjects[i].name);
  }
...
void OnlineGUI::CanvasDraw(drawcommand command,UInt_t padnum) {
  ...
  if(command.directory.IsNull()) {
    ...
    fDir = (TDirectory*)fRootFile->Get(command.directory);
  }
  
  if(fDir) fDir->cd();
  fRootFile->cd();
  TCanvas *tcanv = (TCanvas*)fRootFile->Get(command.objname);
  fCanvas->cd(padnum);
  tcanv->DrawClonePad();
  delete tcanv;
...
void OnlineGUI::GraphDraw(drawcommand command) {
  ...
  if(fDir) fDir->cd();
  TGraph *thisgraph = (TGraph*)fRootFile->Get(command.objname);
...
void OnlineGUI::HistDraw(drawcommand command) {
  ...
  if(command.objtype.Contains("TH1")) {
    ...
    fRootFile->cd();
    if(fDir) fDir->cd();
    mytemp1d = (TH1D*)gDirectory->Get(command.objname);
    if(!mytemp1d) BadDraw(command.variable+" not found");
	...
	fGoldenFile->cd();
	if(fDir) fDir->cd();
	mytemp1d_golden = (TH1D*)gDirectory->Get(command.objname);
	...
	mytemp1d_golden->SetStats(kFALSE);
	mytemp1d_golden->Draw();
	if(!htitle.IsNull()) mytemp1d_golden->SetTitle(htitle);
	mytemp1d->Draw("sames"+type);
      } else {
	mytemp1d->SetStats(showStat);
	if(!htitle.IsNull()) mytemp1d->SetTitle(htitle);
	mytemp1d->Draw(type);
      }
    }
    return;
  ...
  if(command.objtype.Contains("TH2")) {
    ...
    if(fDir) fDir->cd();
    mytemp2d = (TH2D*)gDirectory->Get(command.objname);
	...
	fGoldenFile->cd();
	if(fDir) fDir->cd();
	mytemp2d_golden = (TH2D*)gDirectory->Get(command.objname);
	mytemp2d_golden->SetMarkerColor(2);
	mytemp2d->SetStats(showStat);
	mytemp2d_golden->SetStats(kFALSE);
	mytemp2d_golden->Draw(type);
	if(!htitle.IsNull()) mytemp2d_golden->SetTitle(htitle);
	mytemp2d->Draw("sames"+type);
      } else {
	mytemp2d->SetStats(showStat);
	if(!htitle.IsNull()) mytemp2d_golden->SetTitle(htitle);
	mytemp2d->Draw(type);
  ...
  if(command.objtype.Contains("TH3")) {
    ...
    if(!showGolden) fRootFile->cd();
    if(fDir) fDir->cd();
    mytemp3d = (TH3D*)gDirectory->Get(command.objname);
	...
	fGoldenFile->cd();
	if(fDir) fDir->cd();
	mytemp3d_golden = (TH3D*)gDirectory->Get(command.objname);
	mytemp3d_golden->SetMarkerColor(2);
	mytemp3d->SetStats(showStat);
	mytemp3d_golden->SetStats(kFALSE);
	mytemp3d_golden->Draw(type);
	if(!htitle.IsNull()) mytemp3d_golden->SetTitle(htitle);
	mytemp3d->Draw("sames"+type);
      } else {
	mytemp3d->SetStats(showStat);
	if(!htitle.IsNull()) mytemp3d_golden->SetTitle(htitle);
	mytemp3d->Draw(type);
...
drawcommand OnlineGUI::fileObject2command(drawcommand command) {
  ...
  for(UInt_t i=0; i<vfileObjects.size(); i++) {
    if(vfileObjects[i].name==command.variable ||
       vfileObjects[i].title==command.variable ) {
      command.objname = vfileObjects[i].name;
      command.objtitle = vfileObjects[i].title;
      command.objtype = vfileObjects[i].type;
      command.directory = vfileObjects[i].directory;
...
void OnlineGUI::TreeDraw(drawcommand command) {
  ...
  if(command.treename.IsNull()) {
    ...
    iTree = GetTreeIndexFromName(command.treename);
  }
  TString drawopt = command.type;
  Int_t errcode=0;
  if(drawopt.IsNull() && var.Contains(":")) drawopt = "COLZ";
  ...
  if (iTree <= fRootTree.size() ) {
    ...
    if(errcode==-1) {
      ...
      if(!command.title.IsNull()) {
	TH1* thathist = (TH1*)hobj;
	thathist->SetTitle(command.title);
      }
      if(!showStat) {
	TH1* thathist = (TH1*)hobj;
	thathist->SetStats(kFALSE);
      }
    } else {
      BadDraw("Empty Histogram");
    }
