...
#endif
{
  ...
  orig.GetCoordinate(i);
  const Road::Pvec_t& points = orig.GetCoordinate1();
  DeleteContainerOfContainers( fPoints );
  for (Road::Pvec_t::const_iterator it2 = points.begin();
       it2 != points.end(); ++it2){
    Road::Point* p = *it2;
    //cout << "\t" << p->z << "\t" << p->hit->GetPlaneNum()<<endl;
    Double_t z = p->z;
    UInt_t np = p->hit->GetPlaneNum();
    int i = (p->hit->GetDriftDist() == 0.0) ? 1 : 2;
    do {
      Double_t x = (--i != 0) ? p->hit->GetPosL() : p->hit->GetPosR();
      if( np != last_np ) {
	fPoints.push_back( Pvec_t() );
	last_np = np;
      }
      fPoints.back().push_back( new Point(x, z, p->hit) );
    }while( i );
  }
...
Bool_t Road::IsSame(const Road& example)
  {
    const Road::Pvec_t& points1 = GetPoints();
    const Road::Pvec_t& points2 = example.GetPoints();
   
    Road::Pvec_t::const_iterator it2 = points2.begin();
    for( Road::Pvec_t::const_iterator it1 = points1.begin(); it1 != points1.end(); ++it1 ) {
      //cout << (*it1)->hit->GetWireNum() << "\t" << (*it2)->hit->GetWireNum() << endl;
      if( (*it1)->hit->GetWireNum() != (*it2)->hit->GetWireNum()) return false;
      it2++;
    }
    return true;
  }
...
void Road::Finish()
{
  ...
  //cout << edgpos.front().first << "\t"<<  edgpos.front().second << "\t" << edgpos.back().first << "\t" << edgpos.back().second << "\t" << fProjection->GetType() << "\t" << fProjection->GetPlaneZ(lastpl) << endl;
...
//   fZU = fProjection->GetPlaneZ(5) + eps;
  ...
  // cout << fProjection->GetType() << "\t" << fCornerX[0] << "\t" << fCornerX[1] << "\t" << fCornerX[2] << "\t" << fCornerX[3] << endl;
...
Bool_t Road::Include( const Road* other )
{
  ...
  void Road::GetCoordinate(Int_t i)
  {
    ftempCoord.clear();
    vector<Pvec_t>::size_type npts = fPoints.size();
    Pvec_t selected;
...
Bool_t Road::CollectCoordinates()
{
  ...
  // cout << fCornerX[0] << "\t" << fCornerX[1] << "\t" << fCornerX[2] << "\t" << fCornerX[3] << endl;
  ...
  for( siter_t it = fHits.begin(); it != fHits.end(); ++it ) {
    ...
    if (hit->GetWirePlane()->IsSpecial()){
      Double_t tempdis = (fCornerX[2]+fCornerX[3])/2. - hit->GetWirePos();
      if (tempdis>=hit->GetWirePlane()->GetResolution()*(-1.)&&tempdis<=hit->GetWirePlane()->GetResolution()){
 	hit->Shiftdis(tempdis);
      }
    }
...
// 	fPoints.back().push_back( new Point(x, z, hit) );
    ...
    // The hits are sorted by ascending plane number
    if (qflag==1){
      int i = (hit->GetDriftDist() == 0.0) ? 1 : 1;
      do {
	Double_t x = (--i != 0) ? hit->GetPosL() : hit->GetPosR();
	if( np != last_np ) {
 	  fPoints.push_back( Pvec_t() );
...
Bool_t Road::Fit()
{
  ...
  //cout << fProjection->GetType() << "\t" << CollectCoordinates() << endl;
  if( !fGood )
    return false;
  ...
  vector<Pvec_t>::size_type npts = fPoints.size();
  vector<Pvec_t>::size_type npts_final = npts;
  Pvec_t selected;
  selected.reserve( npts );
  fDof = npts-2;
  pdbl_t chi2_interval = fProjection->GetChisqLimits(fDof);
  vector<Double_t> w(npts);
  Pvec_t::size_type chi2_m1=100,chi2_m2=100;
  vector<Double_t> chi2_p(npts);
  vector<Double_t> S11_p(npts);
  vector<Double_t> S12_p(npts);
  vector<Double_t> S22_p(npts);
  vector<Double_t> G1_p(npts);
  vector<Double_t> G2_p(npts);	
  Double_t chi2;
  
 //  //cout << "Combination: " << "\t" << n_combinations << "\t" << npts << endl;
  ...
  for( UInt_t i = 0; i < n_combinations; ++i ) {
    ...
    Double_t S11 = 0, S12 = 0, S22 = 0, G1 = 0, G2 = 0;
    chi2 = 0;
    for ( Pvec_t::size_type j = 0; j < npts; j++) {
      S11 += S11_p[j];
      S12 += S12_p[j];
    ...
    if( (chi2 < chi2_interval.first || chi2 > chi2_interval.second) && (npts_final>6-fProjection->GetMaxMiss())){
      //cout << chi2_m1 << "\t" << 5 << endl;
      S11_p[chi2_m1]=0.;
      S12_p[chi2_m1]=0.;
      S22_p[chi2_m1]=0.;
	...
	D   = S11*S22 - S12*S12;
	iD  = 1.0/D;
	a1  = (G1*S22 - G2*S12)*iD;  // Intercept
	a2  = (G2*S11 - G1*S12)*iD;  // Slope
	// Covariance matrix of the fitted parameters
	//V[3] = { S22*iD, -S12*iD, S11*iD };
	V[0] = S22*iD;
	V[1] = -S12*iD;
	V[2] = S11*iD;
	max = 0;
	...
	chi2_interval = fProjection->GetChisqLimits(fDof-1);
	npts_final = npts-1;
    }
	...
	Double_t d = a1 + a2*p->z - p->x;
      ...
      chi2_interval = fProjection->GetChisqLimits(fDof-2);
      npts_final=npts -2;
    }
    ...
    Pvec_t selected_final;
    if (npts==npts_final){
      for (Pvec_t::size_type j = 0; j < npts; j++){
	selected_final.push_back(selected[j]);
      }
    }else if (npts==npts_final+1){
      for (Pvec_t::size_type j = 0; j < npts; j++){
	if (j!=chi2_m1) selected_final.push_back(selected[j]);
      }
    }else {
      for (Pvec_t::size_type j = 0; j < npts; j++){
	if (j!=chi2_m1||j!=chi2_m2) selected_final.push_back(selected[j]);
      }
    }
