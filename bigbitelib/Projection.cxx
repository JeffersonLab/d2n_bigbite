...
Projection::Projection( EProjType type, const char* name, Double_t angle,
  ...
  : THaAnalysisObject( name, name ), fType(type), fNlevels(0),
    ...
    fHitpattern(0), fRoads(0),frawRoads(0), fNgoodRoads(0), fRoadCorners(0)
{
  // Constructor
  ...
  R__ASSERT(fRoads);
  frawRoads = new TClonesArray("TreeSearch::Road", 3);
  R__ASSERT(frawRoads);
}
...
//_____________________________________________________________________________
#ifdef VERBOSE
void Projection::PrintNode( const Node_t& node )
{
...
inline
void Projection::PrintNodeP( const Node_t* node )
{
  PrintNode( *node ); 
}
#endif
...
Int_t Projection::MakeRoads()
{
  ...
  for( NodeVec_t::iterator it = fPatternsFound.begin(); it !=
    ...
    Road* rd = new( (*frawRoads)[GetrawNroads()] ) Road(nd1,this);
    //PrintNodeP(&nd1); 
    //cout << "Projections: " << GetType() << endl;
...
// Workaround:
    ...
    if (rd->CollectCoordinates()){
      UInt_t qx = rd->GetNCoordinates();
      for (Int_t i=0;i!=Int_t(qx);i++){
	Road* rd1 = new ( (*fRoads)[GetNroads()] ) Road(*rd,i);
	// Int_t flag=0;
	// 	for (UInt_t qabc=0;qabc!=GetNroads()-1;qabc++){
//  	  if (rd1->IsSame(*((Road*)(*fRoads)[qabc]))) { 
// 	    flag=1;
// 	  }
//  	}
	
// 	if (flag==1) {
// 	  cout << "1: " << GetNroads() << endl;
// 	  fRoads->Remove(rd1);
// 	  cout << "2: " << GetNroads() << endl;
// 	}
      }
    }
