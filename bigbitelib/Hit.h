...
namespace TreeSearch {
  ...
  public:
    Hit() : fWirePlane(0) {}
      Hit( Int_t wnum, Double_t pos, Int_t tdc, Double_t time, Double_t res, Int_t spehit, WirePlane* wp ) :
      fWireNum(wnum), fRawTDC(tdc), fTime(time), fPos(pos), fPosL(pos), 
	fPosR(pos), fResolution(res), fspehit(spehit), fWirePlane(wp)
#ifdef TESTCODE
    ...
    Int_t    Isspecial() const{return fspehit;}
  ...
  public:
    MCHit() : fMCTrack(0) {}
      MCHit( Int_t wnum, Double_t pos, Int_t tdc, Double_t time, Double_t res, Int_t spehit,
	   WirePlane* wp, MCTrack* mctrk, Double_t mcpos )
	: Hit(wnum, pos, tdc, time, res, spehit, wp), fMCTrack(mctrk), fMCPos(mcpos) {}
    virtual ~MCHit() {}
