	...
	if( front_plane->Contains(xf,yf) ) {
	  ...
	  if( back_plane->Contains(xb,yb) ) {
	    ...
	    for( Rvec_t::iterator it=range.first; it != range.second; ++it ) {
	      ...
	      //cout << matchval << "\t" << f3dMatchCut << endl;
#ifdef VERBOSE
	      if( fDebug > 3 ) {
...
Int_t MWDC::CoarseTrack( TClonesArray& tracks )
{
  ...
  if( nproj >= 3 ) {
	...
	fit_par.ndof = FitTrack( these_roads, fit_par.coef, fit_par.chi2 );
	if( fit_par.ndof > 0 ) {
	  //cout << "qx1: " << PassTrackCuts(fit_par) << "\t" << fit_par.chi2 << "\t" << fit_par.ndof << endl;
...
Int_t MWDC::ReadDatabase( const TDatime& date )
{
  ...
  // Try to add the special plane which is for shower detector!!! Xin Qian
  if (fuseshower==1){
    ...
    // qxtemp = "u" + specialplanes[0];
//     name = qxtemp.c_str();
      
//     it = find_if( ALL(fPlanes), WirePlane::NameEquals( name ) );
//     if( it != fPlanes.end() ) {
//       Error( Here(here), "Duplicate plane name: %s. Fix database.", name );
//       return kInitError;
//     }
    
//     SpecialWirePlane* newplane1 = new SpecialWirePlane( name, name, this );
//     newplane1->SetSpecial(1);
//     if( !newplane1 or newplane1->IsZombie() ) {
//       // Urgh. Something is very bad
//       Error( Here(here), "Error creating wire plane %s. Call expert.", name );
//       return kInitError;
//     }
    
//     fPlanes.push_back( newplane1 );
//     newplane1->SetDebug( fDebug );

//     qxtemp = "v" + specialplanes[0];
//     name = qxtemp.c_str();
...
//     SpecialWirePlane* newplane2 = new SpecialWirePlane( name, name, this );
//     newplane2->SetSpecial(1);
//     if( !newplane2 or newplane2->IsZombie() ) {
//       // Urgh. Something is very bad
//       Error( Here(here), "Error creating wire plane %s. Call expert.", name );
//       return kInitError;
//     }
    
//     fPlanes.push_back( newplane2 );
//     newplane2->SetDebug( fDebug );
